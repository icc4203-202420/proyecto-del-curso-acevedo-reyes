class API::V1::BarsController < ApplicationController
  ...
  def index
    ...
    render json: { 
      bars: @bars.as_json(include: {
        address: { 
          include: { country: { only: [:id, :name] } },
          only: [:id, :line1, :line2, :city, :user_id]
        }
      }, 
      only: [:id, :name, :latitude, :longitude, :created_at, :updated_at] )} , status: :ok
  end
  ...
  def show
    if @bar.image.attached?
      render json: { bar: @bar.as_json(include: {
        address: { 
          include: { country: { only: [:id, :name] } },
          only: [:id, :line1, :line2, :city]
        }
      }, except: [:address_id]).merge({ 
